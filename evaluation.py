...
def precision(y_true, y_pred, which=1):
	'''
	Calculate the precision of a classification prediction for a particular class. The precision for a class is equal to the proportion of observations predicted to be of a particular class that are actually of that class, i.e. true positives / predicted positives.
	...
	if predicted_positives != 0:
		...
		warnings.warn('Precision is undefined when predicted_positives == 0. Returning a default value of 0.',
					  RuntimeWarning)
		return 0
...
def recall(y_true, y_pred, which=1):
	'''
	Calculate the recall of a classification prediction for a particular class. The recall for a class is equal to the proportion of observations actually of a class that were also predicted to be of that class, i.e. true positives / all positives.
...
def mean_squared_error(y_true, y_pred):
	'''
	Calculate the mean squared error of a regression prediction, equal to the arithmetic mean of the square of the difference between each ground truth value and the corresponding prediction.
...
def roc_curve(y_true, y_proba):
	...
	fpr: 1D array
		The false positive rates, calculated as false positives / total negatives, corresponding to each threshold in `thresholds`. Also equal to 1 - specificity.

	tpr: 1D array
		The true positive rates, calculated as true positives / total positives, corresponding to each threshold in `thresholds`. Also known as recall.
...
def auc(x, y):
	'''
	Calculate the signed area under a curve by numerical integration.
...
def train_test_split(X, y, random_state):
	'''Split a dataset into training and testing sub-datasets. Quick utility meant to mimic the function of `sklearn.model_selection.train_test_split` by wrapping `Splitter`.
