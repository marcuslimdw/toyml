...
def check_shape(array_like, expected_dim):
	...
	if array_dim != expected_dim:
		...
		if array_dim == 1 and expected_dim == 2:
			raise ValueError('Got a 1D array-like object where a 2D array was expected. If your sample has one observation or one feature, reshape it as a 2D array with one row or column accordingly.')

		else:
			raise ValueError('Got a {}D array-like object where a {}D array was expected.'.format(array_dim, 					 expected_dim))
